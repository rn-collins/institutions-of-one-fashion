...
for(const p of packages){const x=exhibitions[p.id],o=objects.find(v=>v.id===x.objectId),sourceIds=[...new Set([...(claims[p.id]||[]).flatMap(r=>r[2]),...x.movements.flatMap(m=>m[4])])],s1=sources.find(s=>sourceIds.includes(s.id)&&s.url),s2=sources.find(s=>sourceIds.includes(s.id)&&s.url&&s.id!==s1?.id);const judgment=graphicFirst.has(p.id)?'EVIDENCE-GRAPHIC FIRST; IMAGE AS MATERIAL WITNESS':'IMAGE-FIRST HYBRID';add(`/packages/${p.id.toLowerCase()}`,`${p.id} · ${p.title}`,judgment,`${o.title} (${o.id})`,s1?`${s1.title} (${s1.id}) — exact record/interface capture target`:'Exact primary record — unresolved acquisition target',s2?`${s2.title} (${s2.id}) or four-movement evidence graphic`:`Four-movement ${p.domain} evidence graphic`,graphicFirst.has(p.id)?'The institutional relationship is the subject; a diagram is more precise than decorative fashion imagery.':'Acquire a second distinct reusable object before repeating the witness; otherwise use the sourced evidence graphic.');}
